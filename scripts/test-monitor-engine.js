...
// ---- riskStats (4.5): must match the python reference to 1e-9 ----
// Reference generated by scripts/risk-reference.py (identical LCG series,
// identical formulas in an independent language) on 2026-07-26.
{
  // Park-Miller minstd: seed*48271 < 2^53, so python ints and JS doubles
  // agree EXACTLY (the glibc constants overflow JS precision — the gate's
  // first run caught the two languages silently diverging).
  let seed = 42;
  const rnd = () => { seed = (seed * 48271) % 2147483647; return seed / 2147483647; };
  const vals = [100];
  for (let i = 0; i < 500; i++) vals.push(vals[vals.length - 1] * Math.exp((rnd() - 0.5) * 0.04));
  const obs = vals.map((v, i) => ({ date: bday(i), value: v }));
  const rs = R.riskStats(obs);
  const REF = {
    n: 500, annVol: 18.204923532879036, var95: 1.796536586618299, var99: 1.9610375791979093,
    pvar95: 1.8507249648075617, pvar99: 2.632264194923059, mdd: -27.520250702781844,
    skew: -0.039014501016687825, kurtosis: -1.1293632032970806,
  };
  ok(!!rs && rs.n === REF.n, 'riskStats: return count matches reference');
  const close = (a, b) => a != null && Math.abs(a - b) < 1e-9;
  ok(close(rs.annVol, REF.annVol), 'riskStats: annVol matches python (' + rs.annVol.toFixed(6) + ')');
  ok(close(rs.var95, REF.var95), 'riskStats: historical VaR95 matches python');
  ok(close(rs.var99, REF.var99), 'riskStats: historical VaR99 matches python');
  ok(close(rs.pvar95, REF.pvar95) && close(rs.pvar99, REF.pvar99), 'riskStats: parametric VaR matches python');
  ok(close(rs.mdd, REF.mdd), 'riskStats: max drawdown matches python');
  ok(close(rs.worst.ret, -1.9764310818001452) && close(rs.best.ret, 2.019905692721813), 'riskStats: worst/best day match python');
  ok(close(rs.skew, REF.skew) && close(rs.kurtosis, REF.kurtosis), 'riskStats: skew/kurtosis match python');
}
