...
def run(today: str | None = None) -> dict:
    ...
    for p in positions:
        ...
        pnl_pct = (last / avg_cost - 1) if avg_cost else None
        rets[yt] = s.pct_change(fill_method=None).tail(180)
        ...
        # invalidation / cut-loss proximity
        cut_loss = -0.20
        if pnl_pct is not None and pnl_pct <= cut_loss:
            signals.append({
                "asof": today, "desk_id": None, "kind": "book_risk", "ref": p["symbol"],
        ...
        if stop and last <= stop * 1.05:
            signals.append({
                "asof": today, "desk_id": th.get("desk_id") if th else None,
                "kind": "book_risk", "ref": p["symbol"],
    ...
    # factor concentration via PCA on position returns
    concentration = None
    if len(rets) >= 3:
    ...
    if signals:
        db.upsert("research", "signal", signals, on_conflict="dedupe_key,asof")
