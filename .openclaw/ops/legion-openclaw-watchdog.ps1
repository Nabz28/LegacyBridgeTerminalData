...
function Test-GatewayHealth {
  ...
  $failureText = (($healthProbe.Stdout, $healthProbe.Stderr) -join "`n").Trim()
  $stallReasonAfterProbeFailure = Test-RecentStallSignature
  if ($stallReasonAfterProbeFailure) {
    return @{ Healthy = $false; Reason = $stallReasonAfterProbeFailure; Raw = $failureText }
  }

  if ($healthProbe.TimedOut) {
    return @{ Healthy = $false; Reason = "gateway health probe timed out while tcp open"; Raw = $failureText }
  }

  return @{ Healthy = $false; Reason = "gateway health probe failed while tcp open"; Raw = $failureText }
}
