...
- Text handling works through OpenClaw/Codex and Telegram delivery.
- Telegram image auto-understanding is temporarily disabled in
  `~\.openclaw\openclaw.json`; on 2026-05-30 the Codex media-understanding
  provider hung on `model/list` and wedged the gateway.
- Watchdog runs every 5 minutes via Windows Scheduled Task and hard-kills the
  gateway immediately when `openclaw gateway restart` times out.
...
2. Stable image-understanding provider/API key so Telegram images can be
   re-enabled without freezing replies.
3. Provider fallback so quota does not halt LEGION.
4. Live group Telegram validation.
5. Dedicated-number WhatsApp activation.
6. Periodic brain-backed health reporting.
