...
- Channel parked: WhatsApp, disabled until the dedicated eSIM WhatsApp account
  is stable.
- Model backend: Codex OAuth, `openai/gpt-5.5` as the OpenClaw agent model via
  Codex auth.
- Image auto-understanding: temporarily disabled after the 2026-05-30 stall.
  `codex/gpt-5.5` media describe can hang during provider `model/list` and
  wedge the gateway. Re-enable only after adding a stable image provider/API key
  or proving the Codex provider no longer stalls.
...
Image prompt remains in the config as the intended bounded description path, but
`tools.media.image.enabled` is currently `false`. This is deliberate. When it was
enabled with the Codex provider, Telegram image intake stalled on `model/list`,
left the session in `processing/embedded_run`, and stopped replies.
...
- Text smoke returns `alive`, gateway health returns `OK`, and Telegram channel
  status shows `running, connected`. Image smoke is expected to stay disabled
  until a stable vision provider is added.
...
- If normal restart fails, kills only the OpenClaw gateway Node process and
  starts it again.
- If `openclaw gateway restart` times out, skips the long normal wait and
  immediately forces the gateway process restart.
...
- Send text or reminders.
- Images are currently received by Telegram, but automatic image understanding is
  disabled to protect reply reliability. Do not claim LEGION can read a fresh
  Telegram image until image auto-understanding is re-enabled and smoke-tested.
...
- Codex subscription quota can block replies.
- Telegram image auto-understanding is disabled because Codex media describe
  wedged the gateway on 2026-05-30. Next fix is a stable vision provider/API key.
