...
(function (global) {
  ...
  function loadDb() {
    if (!global.initSqlJs) {
      ...
      setStatus('statusDb', 'err', 'Database: sql.js missing');
      return Promise.reject(err);
    }
    setStatus('statusDb', 'warn', 'Database: loading…');
    return Promise.all([
      global.initSqlJs({ locateFile: function (f) { return SQL_WASM_BASE + f; } }),
      fetch(paths().sqlite).then(function (r) {
        if (!r.ok) throw new Error('ricdb http ' + r.status);
        return r.arrayBuffer();
      })
    ]).then(function (results) {
      sql = results[0];
      db = new sql.Database(new Uint8Array(results[1]));
      var info = db.exec('SELECT COUNT(*) AS n_series FROM series');
      var nSeries = info && info[0] ? info[0].values[0][0] : 0;
      setStatus('statusDb', 'ok', 'Database: ' + nSeries.toLocaleString() + ' series');
      return db;
    }).catch(function (err) {
  ...
  function _seriesForRic(ric) {
    if (!db) return null;
    var stmt = db.prepare('SELECT date, value FROM observations WHERE ric=$ric ORDER BY date');
    stmt.bind({ $ric: ric });
    var out = [];
    while (stmt.step()) {
      var row = stmt.get();
      out.push({ date: row[0], value: row[1] });
    }
    stmt.free();
    return out;
  }
  ...
  function getForecastErrors() {
    ...
    FORECAST_PAIRS.forEach(function (pair) {
      var fSeries = _seriesForRic(pair.forecast) || [];
      var aSeries = _seriesForRic(pair.actual) || [];
      var pairs = _alignByNearestDate(aSeries, fSeries, 540);  // up to 18mo distance
      if (!pairs.length) {
